...
def Tester(model,
    ...
    chkpoint_backbone = torch.load(f'./{experiment_log_dir}/saved_models/ckp_best.pt')
    chkpoint_classer = torch.load(f'./{experiment_log_dir}/saved_models/ckp_best_classer.pt')
    ...
    ft_model.load_state_dict(chkpoint_backbone['model_state_dict'])
    ft_classifier.load_state_dict(chkpoint_classer['model_state_dict'])
    test_loss, test_acc, test_auc, test_prc, emb_test, label_test, performance, confusion_matrix_test = model_test(ft_model,
                                                                                            test_dl,
                                                                                            device,
    ...
    if best_performance is None:    
        ...
        if performance[0] > best_performance[0]:
            ...
            logger.debug(
                ...
                1, performance[0], performance[1], performance[2], performance[3]))

    logger.debug("Fine-tuning ended ....")
    logger.debug("=" * 100)
    logger.debug('EP%s - Best Testing: Acc=%.4f| Precision = %.4f | Recall = %.4f | F1 = %.4f' % (
             1, best_performance[0], best_performance[1], best_performance[2], best_performance[3]))
    logger.debug("=" * 100)
    logger.debug(confusion_matrix_test)
...
def model_test(model, test_dl, device, classifier=None, experiment_log_dir=None):
    model.eval()
    classifier.eval()
    ...
    with torch.no_grad():
        ...
        for data, labels in test_dl:
            ...
            if flag==0:
                prob_all = predictions_test
                label_all = labels
            else:
                prob_all = torch.cat((prob_all, predictions_test), axis=0)
                label_all = torch.cat((label_all, labels), axis=0)
    ...
    prob_all = prob_all.cpu().numpy()
    label_all = label_all.cpu().numpy()
    # save the predicted probabilities and labels for all test samples
    np.save(f'./{experiment_log_dir}/saved_models/prob_test.npy', prob_all)
    np.save(f'./{experiment_log_dir}/saved_models/labels_test.npy', label_all)

    labels_numpy_all = labels_numpy_all[1:]
    pred_numpy_all = pred_numpy_all[1:]
    confusion_matrix_test = confusion_matrix(pred_numpy_all, labels_numpy_all)
