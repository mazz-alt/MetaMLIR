...
import numpy as np
from sklearn.metrics import roc_auc_score, average_precision_score, accuracy_score, precision_score, f1_score, recall_score, confusion_matrix
from utils.augmentations import data_transform_masked4cl
from model import Backbone, Inverted_Encoding_CHead
import warnings
warnings.filterwarnings("ignore")

def build_model(args, lr, configs, device, chkpoint=None):
    # Model Backbone
    model = Backbone(configs, args).to(device)
    if chkpoint:
    ...
    # Classification Head
    classifier = Inverted_Encoding_CHead(configs).to(device)
...
def Trainer(model_all,
    ...
    for epoch in range(1, args.pretrain_epoch + 1):
        ...
        train_dl = train_dl_all[model_num]
        configs = configs_all[0]  
        model = model_all[model_num]
        model_optimizer = model_optimizer_all[model_num]
        model_scheduler = model_scheduler_all[model_num]

        pretrain=True
        total_loss, total_cl_loss, total_rb_loss = model_pretrain(model, model_optimizer, model_scheduler, train_dl, pretrain, configs, args, device)

        logger.debug(
            f'Pre-training Epoch: {epoch}\t Train Loss: {total_loss:.4f}\t CL Loss: {total_cl_loss:.4f}\t RB Loss: {total_rb_loss:.4f}\n')

        chkpoint = {'seed': seed, 'epoch': epoch, 'train_loss': total_loss,  'model_number':model_num, 'model_state_dict': model.state_dict()}
    ...
    for ep in range(1, args.finetune_epoch + 1):
        ...
        if ep % args.log_epoch == 0:
            ...
            test_loss, test_acc, test_auc, test_prc, emb_test, label_test, performance, confusion_matrixn_test = model_test(ft_model,
                                                                                                    test_dl,
                                                                                                    pretrain, 
                                                                                                    device,
                                                                                                    classifier=ft_classifier)
            ...
            if best_performance is None:
                ...
                if performance[0] > best_performance[0]:
                    ...
                    chkpoint2 = {'seed': seed, 
                                'model_state_dict': ft_classifier.state_dict() }
                    torch.save(chkpoint1, os.path.join(experiment_log_dir, f"saved_models/", f'ckp_best.pt'))
                    torch.save(chkpoint2, os.path.join(experiment_log_dir, f"saved_models/", f'ckp_best_classer.pt'))
        
    logger.debug("Fine-tuning ended ....")
    logger.debug("=" * 100)
    logger.debug('EP%s - Best Testing: Acc=%.4f| Precision = %.4f | Recall = %.4f | F1 = %.4f' % (
    ep, best_performance[0], best_performance[1], best_performance[2], best_performance[3]))   # gai ep  epoch
    logger.debug("=" * 100)
    logger.debug(confusion_matrixn_test)

    return best_performance


def model_pretrain(model, model_optimizer, model_scheduler, train_loader, pretrain, configs, args, device):
    total_loss = []
    total_cl_loss = []
    total_rb_loss = []
    ...
    for batch_idx, (data, labels) in enumerate(train_loader):
        ...
        loss, loss_cl, loss_rb= model(data_masked_om, pretrain)
...
def model_finetune(model, val_dl, pretrain, device, model_optimizer, model_scheduler, classifier=None, classifier_optimizer=None):
    model.train()
    classifier.train()
...
def model_test(model, test_dl, pretrain, device, classifier=None):
    model.eval()
    classifier.eval()
    ...
    confusion_matrix_test = confusion_matrix(labels_numpy_all,pred_numpy_all).T
    ...
    return total_loss, total_acc, total_auc, total_prc, emb_test_all, trgs, performance, confusion_matrix_test
